...
knitr::kable(head(healthSpending), caption = "Health Expenditures by Country")
```

```{r, results='asis',message=FALSE, warning=FALSE}
knitr::kable(head(lifeExpect), caption = "Life Expectancy by Country")
```
...
```{r}
source("../02 Data Wrangling/life_vs_health.R")
```
